class TickerFetcherJob < ApplicationJob
  ...
  def perform pair_id
    ...
    if pair.market.is_watched && pair.is_watched
      ...
      if pair.exchange.last_ticker_request && (pair.exchange.last_ticker_request > (DateTime.current - 5.seconds)) # last_ticker_request Happened less than 15 seconds ago
        # Too early to perform request yet
        next_request = DateTime.current + 5.seconds  # Trying to fullfill its destiny every 15 seconds
      else
        pair.get_ticker
        next_request = DateTime.current + random_sec.seconds# + 15.seconds # Request to exchange for pair_id every 3 minutes
      end
